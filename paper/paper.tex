...
The project registers two AlpaSim model entry points:
\texttt{token\_dagger\_bc} and \texttt{direct\_actor\_planner}. Each implements AlpaSim's
...
internals: it is the shared policy input used by both AlpaSim plugins.
...
The direct planner adapter and shared contract utilities return trajectory arrays,
headings, and structured reasoning payloads through the AlpaSim model API without
learned-checkpoint runtime requirements. \\
...
the adapter contract; any policy benchmark claim requires executed scenes, baselines,
and complete failure reporting.}
...
closed-loop harness for learned token and actor-aware planning policies,
